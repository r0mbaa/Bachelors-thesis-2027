...
| `layout` | Модель планировки, ячейки, мастер типовой планировки, граф склада по §8.7, валидация, классификатор топологии | Готово для алгоритмов; дальше — только по потребностям планировщика |
| `core` | Весь учётный путь: приёмка → размещение → заказ → резерв → задание → терминал → отгрузка; пользователи и роли, аудит; мастер, проверка и граф в API конструктора; заполнение склада по правилу; этикетки и маршрутный лист в PDF; OpenAPI | Остальные печатные формы, слоттинг, монитор по WebSocket, вызов планировщика |
| Развёртывание | `docker compose up` с HTTPS, страница проверки камеры телефона | Веб-панель и терминал за тем же прокси |
| `planner-engine`, `planner`, `research` | Каркас | Алгоритмы §9, стенд экспериментов |
| `web`, `terminal` | — | Конструктор, 3D-сцена, PWA-терминал |

Подробный ход работы — в [журнале разработки](../Ход%20разработки.md).
...
**Стенд целиком:** `docker compose up --build` — PostgreSQL, `core`, `planner` и HTTPS-прокси; проверка камеры телефона — `https://<хост>/check/`. Подробности, установка сертификата на телефон и переменные — в [`deploy/README.md`](deploy/README.md).

Запуск учётного ядра для разработки, переменные окружения и первый вход описаны в [`core/README.md`](core/README.md). Сборку и тесты при каждом пуше выполняет GitHub Actions ([`.github/workflows/build.yml`](../.github/workflows/build.yml)).

Для `core` нужен запущенный Docker. Интеграционные тесты поднимают PostgreSQL 16 через Testcontainers, а `bootRun` сам запускает его из [`docker-compose.dev.yml`](docker-compose.dev.yml). Встраиваемая БД не подходит, потому что инварианты учёта держатся на CHECK-ограничениях, триггерах и блокировках строк PostgreSQL (§12.2).
